$(document).ready(function(){
	...
	//product() is a funtion fetching product record from database whenever page is load
		function product(){
		var prodNo = $("#show option:selected").val();
		$.ajax({
			url	:	"action.php",
			method:	"POST",
	...
	function getreviews(){
		...
		$("#reviews").click(function() {
			prod_id = location.search.substring(1).split("=")[1];
			$.ajax({
				url	:	"reviews.php",
				method:	"POST",
	...
	function addreviews(){
		...
		$("#submitReview").click(function() {
			...
			// alert(prod_id+" "+name+" "+ email+" "+ content+" "+ date+" "+rating);
			$.ajax({
				url	:	"reviews.php",
				method:	"POST",
				data	:	{prod_id:prod_id,name:name,email:email,content:content,rating:rating},
				success	:	function(data){
					location.reload();
				}
			})
